...
function led.getPath()
  local name = led.getName()
  local path = "/sys/class/leds/" .. tostring(name) .. "/brightness"
  return path
end
...
function api.call(action, params, aliasId)
  ...
  if b == 200 then
    ...
    if b == "timeout" or b == 404 or b == 500 or b == "connection refused" then
      debugger.log("i detected that the following error occured so i mark this endpoint as bad: " .. b)
...
function getServerById(serverId)
  ...
  if result == nil then
    debugger.log("getServerById(" .. tostring(serverId) .. ") - warning: returning nl")
  end
