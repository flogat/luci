...
local base64 = require "luci.shellfirebox.base64"
...
function sendLogToShellfire()
  ...
  log_content = base64.encode(log_content)
...
function setServerTo(section)
  ...
  if reconnect then
    ...
    setBlockConnectionStateUpdate(false)
    if currentConnectionState ~= "connectionModeChange" then
      setConnectionState("processDisconnected")
    end
  end
...
function connectOpenVpn()
  ...
  if connectionMode == "3" then
    ...
    paramsToAdd = paramsToAdd .. " --connect-retry-max 3 --socks-proxy 127.0.0.1 " .. obfsProxyLocalPort
